...
Релизные сборки используют MinVer. Тег Git вида `v1.7.2` становится версией приложения,
а само приложение корректно парсит `AssemblyInformationalVersion`, поэтому суффиксы
MinVer вроде `-alpha` или `+commitSha` не ломают отображение версии в окне «О программе»
и сравнение версий при проверке обновлений.
